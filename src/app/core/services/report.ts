...
export class ReportService {
  ...
  // 📊 MÉTODOS PARA EL DASHBOARD ANALÍTICO
  getDistrictRanking(): Observable<any[]> {
    return this.http.get<any[]>(`${this.url}/ranking-distritos`);
  }

  getHatcheryRanking(): Observable<any[]> {
    return this.http.get<any[]>(`${this.url}/reportes-tipo-criadero`);
  }

  getStatusRanking(): Observable<any[]> {
    return this.http.get<any[]>(`${this.url}/reporte-por-estado`);
  }

  getReportsWithMostSymptoms(): Observable<any[]> {
    return this.http.get<any[]>(`${this.url}/con-mas-sintomas`);
  }
}